...
if [ "$flag" == "blue" ]; then
    ...
    ./launch_heron.sh l b2 b1 -s --start-x=20 --start-y=35 --start-a=60 --role=DEFEND_MED $@
else
    # Red Gus
    ./launch_heron.sh g r1 r2 -s --start-x=140 --start-y=45 --start-a=240 --role=DEFEND_MED $@ &
    ./launch_heron.sh h r2 r1 -s --start-x=140 --start-y=35 --start-a=240 --role=DEFEND_MED $@
fi
